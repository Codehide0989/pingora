...
Complete guide for running Pingora with Docker
...
# Should show:
# pingora-workflows  | Running database migrations...
# pingora-workflows  | Migrated successfully
# pingora-workflows  | Starting workflows service...
```
...
This creates:
- 3 workspaces (`love-pingora`, `test2`, `test3`)
- 5 sample monitors and 1 status page with slug `status`
- Test user account: `ping@pingora.dev`
...
2. Use magic link authentication with email: `ping@pingora.dev`
3. Check your console/logs for the magic link (with `SELF_HOST=true` in `.env.docker`)
4. After logging in, you'll see the `love-pingora` workspace with all seeded monitors and status page
...
docker inspect pingora-dashboard --format='{{.State.Health.Status}}'
```

## Getting Help

- **Documentation**: [docs.pingora.dev](https://docs.pingora.dev)
- **Discord**: [pingora.dev/discord](https://www.pingora.dev/discord)
- **GitHub Issues**: [github.com/pingoraHQ/pingora/issues](https://github.com/pingoraHQ/pingora/issues)
