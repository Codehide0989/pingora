...
This directory contains community-contributed themes for pingora status pages. These themes allow users to customize the visual appearance of their status pages with different color schemes and design styles.
...
- **Openstatus** - The standard pingora theme
- **Openstatus (Rounded)** - The rounded pingora theme (similar to the legacy page)
...
- Configure your [themes.pingora.dev](https://themes.pingora.dev/?b=true) and copy the configuration
- Directly test by running it locally

### 1. Run the project

Start by forking the pingora repository to your GitHub account and run the command `dev:status-page` locally following [Getting Started](https://github.com/pingoraHQ/pingora?tab=readme-ov-file#getting-started-) steps.

Once you run the `@pingora/status-page` app, e.g. via the following command in the root directory:
...
Need help creating a theme? Open an issue or reach out to the pingora community!