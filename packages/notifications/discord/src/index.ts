...
import { discordDataSchema } from "@pingora/db/src/schema";
...
const postToWebhook = async (content: string, webhookUrl: string) => {
  const res = await fetch(webhookUrl, {
    ...
    body: JSON.stringify({
      ...
      username: "Pingora Notifications",
    }),
  });
  if (!res.ok) {
    throw new Error(
  ...
  try {
    await postToWebhook(
      `**🚨 Alert [${name}](<${monitor.url}>)**\nStatus Code: ${
        ...
        cronTimestamp,
      ).toISOString()})\n> Check your [Dashboard](<https://www.pingora.dev/app/>).\n`,
      webhookUrl,
    );
  } catch (err) {
    console.error(err);
