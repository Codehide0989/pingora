...
export class EmailClient {
  ...
  public async sendFollowUp(req: { to: string }) {
    ...
    try {
      ...
      const result = await this.client.emails.send({
        from: "Thibault Le Ouay Ducasse <welcome@pingora.dev>",
        replyTo: "Thibault Le Ouay Ducasse <thibault@pingora.dev>",
  ...
  public async sendFollowUpBatched(req: { to: string[] }) {
    ...
    const result = await this.client.batch.send(
      req.to.map((subscriber) => ({
        from: "Thibault Le Ouay Ducasse <thibault@pingora.dev>",
        subject: "How's it going with Pingora?",
        to: subscriber,
        html,
      })),
        ...
        try: () =>
          this.client.batch.send(
            recipients.map((subscriber) => ({
              from: `${req.pageTitle} <notifications@notifications.pingora.dev>`,
  ...
  public async sendTeamInvitation(req: TeamInvitationProps & { to: string }) {
    ...
    try {
      ...
      const result = await this.client.emails.send({
        from: `${
          req.workspaceName ?? "Pingora"
        } <notifications@notifications.pingora.dev>`,
  ...
  public async sendMonitorAlert(req: MonitorAlertProps & { to: string }) {
    ...
    try {
      ...
      const result = await this.client.emails.send({
        from: "Pingora <notifications@notifications.pingora.dev>",
  ...
  public async sendPageSubscription(
    ...
    try {
      ...
      const result = await this.client.emails.send({
        from: "Status Page <notifications@notifications.pingora.dev>",
