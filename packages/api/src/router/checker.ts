...
import {
  ...
  statusAssertion,
  textBodyAssertion,
} from "@pingora/assertions";
import { and, db, eq } from "@pingora/db";
import { monitor, selectMonitorSchema } from "@pingora/db/src/schema";
import { monitorRegionSchema } from "@pingora/db/src/schema/constants";
import {
...
export async function testHttp(input: z.infer<typeof httpTestInput>) {
  ...
  if (input.url.includes("pingora.dev")) {
    throw new TRPCError({
      code: "BAD_REQUEST",
  ...
  try {
    const res = await fetch(
      `https://pingora-checker.fly.dev/ping/${input.region}`,
      {
        method: "POST",
        headers: {
...
export async function testTcp(input: z.infer<typeof tcpTestInput>) {
  try {
    const res = await fetch(
      `https://pingora-checker.fly.dev/tcp/${input.region}`,
      {
        method: "POST",
        headers: {
          Authorization: `Basic ${env.CRON_SECRET}`,
...
export async function testDns(input: z.infer<typeof dnsTestInput>) {
  try {
    const res = await fetch(
      `https://pingora-checker.fly.dev/dns/${input.region}`,
      {
        method: "POST",
        headers: {
          Authorization: `Basic ${env.CRON_SECRET}`,
...
function generateUrl({ row }: { row: z.infer<typeof selectMonitorSchema> }) {
  switch (row.jobType) {
    case "http":
      return `https://pingora-checker.fly.dev/checker/http?monitor_id=${row.id}`;
    case "tcp":
      return `https://pingora-checker.fly.dev/checker/tcp?monitor_id=${row.id}`;
    case "dns":
      return `https://pingora-checker.fly.dev/checker/dns?monitor_id=${row.id}`;
    default:
