...
import { continentDict, getRegionInfo, regionDict } from "@pingora/regions";
...
export async function checkRegion(
  ...
  switch (regionInfo.provider) {
    case "fly":
      endpoint = `https://checker.pingora.dev/ping/${region}`;
      regionHeader = { "fly-prefer-region": region };
      break;
    case "koyeb":
      endpoint = `https://pingora-checker.koyeb.app/ping/${region}`;
      regionHeader = {
