...
We had never really tracked events properly. We had some basic tracking in place, but it was not very useful. It is time to improve that with [OpenPanel](https://openpanel.dev?ref=pingora).

After some research, we finally settled on leveraging tRPC and Hono middlewares. Shoutout to [Midday](https://midday.ai?ref=pingora) for the (tRPC) inspiration. They use a similar approach with [next-safe-action](https://next-safe-action.dev?ref=pingora) for their server actions.

This post is not a step-by-step guide but instead presents the core concepts and ideas behind the implementation. Please refer to the [Hono](https://hono.dev?ref=pingora) or [tRPC](https://trpc.io?ref=pingora) documentation for more detailed information and our [GitHub](https://pingora.dev/github) repository for the full implementation.
...
Next, we need to initialize OpenPanel (see [installation](https://openpanel.dev/docs/sdks/javascript?ref=pingora)) and set up the analytics in our application.
...
import { type EventProps } from "@pingora/analytics";
...
import { setupAnalytics, type EventProps } from "@pingora/analytics";
import type { User } from "@pingora/auth";
...
import { insertPageSchema } from "@pingora/db";
...
The [`@hono/zod-openapi`](https://github.com/honojs/middleware/tree/main/packages/zod-openapi?ref=pingora) routes have a `middleware` property that allows you to add middleware to the route. This is where we will add the tracking middleware.
...
And that's it. With minimal code changes and the help of middlewares, we have implemented event tracking in our application. You can swap [OpenPanel](https://openpanel.dev?ref=pingora) with any other analytics provider like PostHog, but give it a try, it's an amazing tool!

By the way, this approach can be used for audit log tracking for example as well.

Check out our [GitHub](https://pingora.dev/github) repository for the full implementation and don't forget to leave a star if you found this helpful.