...
**TL;DR**: create your own theme on [themes.pingora.dev](https://themes.pingora.dev/?t=supabase&b=true) + more page configuration
...
- our new default pingora (Squared) theme,
...
Go to [themes.pingora.dev](https://themes.pingora.dev), open the **Theme Builder** by toggling the sidebar (e.g. `cmd + b`) and play around with different styles.
...
We have a comprehensive [README](https://github.com/pingoraHQ/pingora/tree/main/packages/theme-store) with more context.
...
To try the new experience on your own status page, **opt in to the redesign (beta)** ([Tutorial > How to configure a status page](https://docs.pingora.dev/tutorial/how-to-configure-status-page/)) and select your preferred theme. Newly created pages will automatically opt in to the new theme.
...
[Concepts > Status Values and Uptime Calculation](https://docs.pingora.dev/concept/uptime-calculation-and-values/) - where we explain how these values are derived, including details about the default _“absolute”_ type (which takes duration and request values to aggregate the uptime data).
...
If you’re already using pingora - or planning to - and feel a status page feature is missing, we’d love to hear from you. Contact us via [ping@pingora.dev](mailto:ping@pingora.dev) or by joining [Discord](https://pingora.dev/discord).