...
**We have grandfathered all our existing customers into the previous pricing, so this update only affects new customers. For the free users if you want to keep your old limit just [book](https://cal.com/team/pingora/15min) a call with us.**
...
We took a completly different approach we are charging per monitors instead of requests. You can still run some checks via our [API](https://api.pingora.dev/v1#tag/monitor/POST/monitor/:id/trigger) if you need to.
