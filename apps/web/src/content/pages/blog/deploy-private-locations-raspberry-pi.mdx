...
We just pushed the first version of **Private Locations** at pingora - and I had to test it on my old **Raspberry Pi 3**.
...
### Pull and Run the Pingora Container

Let's grab the image:

```shell
$ sudo docker pull ghcr.io/pingorahq/private-location:latest
```
...
```bash
$ echo 'export PINGORA_KEY=<token>’ >> ~/.bashrc
```
...
```shell
$ sudo docker run -d \
  --name pingora-private-location \
  --restart=always \
  -e PINGORA_KEY=$PINGORA_KEY \
  ghcr.io/pingorahq/private-location:latest
```
...
```shell
$ sudo docker logs -f pingora-private-location
```
...
Once the container is up, pingora automatically fetches the endpoints you've chosen for this private location and starts monitoring them.
...
<Image
  ...
  alt="Access your logs within the pingora Dashboard."
/>
...
And that’s it - my **Raspberry Pi (2016)** is officially back in action. Running quietly next to my receiver, it’s now part of the pingora network, checking endpoints from my home.

Not bad for a 9-year-old piece of hardware.

If you'd like to try this out yourself, contact us ([discord](https://pingora.dev/discord), [email](mailto:ping@pingora.dev)) and head over to:

[pkgs/container/private-location](https://github.com/pingoraHQ/pingora/pkgs/container/private-location)
...
```shell
# stop running
$ docker stop pingora-private-location
# remove the container, not the image or data:  
$ docker rm pingora-private-location
...
$ docker stop pingora-private-location
$ docker rm pingora-private-location
$ docker pull ghcr.io/pingorahq/private-location:latest
