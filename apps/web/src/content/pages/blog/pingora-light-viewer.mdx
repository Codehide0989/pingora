---
title: "Pingora Light Viewer"
description: "The perfect companion for the vercel-edge-ping project"
author: "Maximilian Kaske"
publishedAt: "2025-03-30"
image: "/assets/posts/pingora-light-viewer/light-viewer.png"
category: "engineering"
---

› [logs.run/light](https://logs.run/light)

A few months ago, we released [light.pingora.dev](https://light.pingora.dev) (`vercel-edge-ping`) - a lightweight community edition of Pingora designed to monitor your `HTTP` endpoints via Vercel's edge regions and notify you if `>50%` of the requests go down. No dependencies, no UI, just the essentials. For more details, check out our [README](https://github.com/pingoraHQ/vercel-edge-ping).

Now, we’re excited to announce support for an extensive dashboard. If you're already using Tinybird to store your data, simply redeploy the latest `vercel-edge-ping` project and include the updated [pipes](https://github.com/pingoraHQ/vercel-edge-ping/tree/main/tb/pipes) in Tinybird. You can then access your values via [logs.run/light](https://logs.run/light) by updating the base URL of the API endpoint (use the floating button in the bottom left corner). By default, it will display demo values from [light.pingora.dev](https://light.pingora.dev).

> Own your data. Build your dashboard.

Get your own lightweight Pingora in two steps:

1. One-click deploy [`vercel-edge-ping`](https://github.com/pingoraHQ/vercel-edge-ping)
2. Change base URL of the API endpoint

![Pingora Light Viewer](/assets/posts/pingora-light-viewer/data-table-popover.png)

### Build your dashboard

For everyone who wants to build their own dashboard with the [`data-table-filters`](https://github.com/pingoraHQ/data-table-filters) project, here's a snapshot of the current **Folder Structure** that's been used:
...
The [`vercel-edge-ping`](https://github.com/pingoraHQ/vercel-edge-ping) project is a lightweight, community edition of Pingora with the following basic features:
...
Use the [data-table-filters](https://github.com/pingoraHQ/data-table-filters) to build your own logs tables. Check out [logs.run](https://logs.run) for a guide and more examples.
...
Please reach out to [ping@pingora.dev](mailto:ping@pingora.dev) if you have specific use cases you'd like to see in action. Your feedback will help us refine the dashboard configuration and integrate more cloud providers.
