...
This article details how to deploy a new Hono server on Vercel and monitor it using Pingora, with a focus on observing the impact of Vercel's Fluid Compute.
...
Next, we configured monitoring with pingora. We created a new monitor with the following YAML configuration.
...
"hono-cold":
  ...
  retry: 3
```
We have deployed it using the [pingora cli](https://docs.pingora.dev/tutorial/get-started-with-pingora-cli/).
...
_Create an account on [Pingora](/app/sign-up) to
