...
With a few days delay exactly two years ago (30.07.2023), we launched pingora with Thibault. Today marks the day we sit together for the second time in real life and are launching the new version of the dashboard. _What a blast!_
...
We are dropping a new dashboard template free to use! The [`pingora-template`](https://github.com/PingoraHQ/pingora-template) is a **full shadcn x Nextjs SPA (so BYO router) starter kit** with some dashboard components ready to use via shadcn CLI.

https://template.pingora.dev
...
```bash
pnpm dlx shadcn@latest add https://template.pingora.dev/r/form-card.json
```
...
Refer to the [README](https://github.com/pingoraHQ/pingora-template) for more informations.
...
Read more about how we worked on our [product strategy](https://pingora.dev/blog/product-strategy-a-reality-check) with Emily Omier.
...
Read more about the latest changes in our newly published [blog post](/blog/introducing-pingora-cli).
