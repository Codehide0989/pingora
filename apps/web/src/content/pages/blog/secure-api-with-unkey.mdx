...
### How does Pingora use Unkey?

Whenever Pingora creates an API key, we will send a request to the Unkey API
...
[GitHub](<https://github.com/pingoraHQ/pingora/blob/main/apps/web/src/app/app/(dashboard)/%5BworkspaceSlug%5D/settings/_components/api-keys/actions.ts>))
...
code source on
[GitHub](https://github.com/pingoraHQ/pingora/tree/main/apps/server).
...
`/api/v1/*`. We will use the `x-pingora-key` request header to append the API
...
--header 'x-pingora-key: os_1234567890'
```

For Pingora, we are running our Hono.js server on [fly.io](https://fly.io)
