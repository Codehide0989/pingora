...
WORKDIR /home/pingora

COPY entrypoint.sh .

RUN  curl -o cli.tar.gz -L  https://github.com/pingoraHQ/cli/releases/latest/download/cli_Linux_x86_64.tar.gz

RUN tar -xf ./cli.tar.gz

ENTRYPOINT ["/home/pingora/entrypoint.sh"]
```
...
```
https://github.com/pingoraHQ/cli/releases/latest/download/cli_Linux_x86_64.tar.gz
```
...
if [ -z "$INPUT_CONFIG_PATH" ]; then
    /home/pingora/pingora run --access-token $INPUT_API_KEY
else
    /home/pingora/pingora run --access-token $INPUT_API_KEY --config $INPUT_CONFIG_PATH
fi

if [ $? -eq 0 ]
then
    echo "Pingora run successfully"
    exit 0
else
    echo "Pingora run failed"
    exit 1
fi
```
...
```yaml
name: 'Pingora Synthetics CI'
description: 'Run your Pingora synthetics checks as part of your GitHub Actions workflow.'
author: 'Pingora'
branding:
  icon: 'zap'
  color: gray-dark
...
inputs:
  api_key:
    description: 'Pingora API key'
    required: true
  config_path:
    description: 'Path to the Pingora configuration file'
    required: false

runs:
  using: docker
  image: docker://ghcr.io/pingorahq/action:latest
  args:
    - ${{ inputs.api_key }}
    - ${{ inputs.config_path }}
```
...
[https://github.com/marketplace/actions/pingora-synthetics-ci](https://github.com/marketplace/actions/pingora-synthetics-ci
)


If you want to see a GitHub repository with the action you can find it here.

[https://github.com/pingoraHQ/github-action-tester/actions](https://github.com/pingoraHQ/github-action-tester/actions
)


_Create an account on [Pingora](/app/sign-in?ref=blog-github-action) and start running your own Synthetics Tests in your GitHub Actions._