...
description: Technical specification for incident status updates within Pingora.
---

## Overview

A status report is a chronological update or event associated with an ongoing incident in Pingora. These reports are crucial for communicating the progress of an incident, from initial detection to final resolution, providing transparency to stakeholders.
