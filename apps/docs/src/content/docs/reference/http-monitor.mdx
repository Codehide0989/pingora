...
**Note:** pingora automatically adds `User-Agent: pingora/1.0` to all requests.
...
- `false`: Monitor data remains private, accessible only within your Pingora dashboard.
...
- **[Monitoring Overview](/monitoring/overview)** - Conceptual overview of Pingora monitoring capabilities.
...
- **[CLI Reference](/reference/cli-reference)** - Guide to managing Pingora monitors programmatically using the command-line interface.
