...
pingora provides embeddable status badges that you can add to any Markdown file, including your `README.md`. You can choose between a modern SVG badge or a legacy PNG badge.

## Prerequisites

-   An pingora account with a configured status page.
-   The "slug" of your status page (the unique name in its URL, e.g., `https://[slug].pingora.dev`).
...
```
https://[slug].pingora.dev/badge/v2
```

##### Markdown Snippet

```
![Status](https://[slug].pingora.dev/badge/v2)
```

**Example:**

![Status](https://status.pingora.dev/badge/v2)
...
```
![Status](https://[slug].pingora.dev/badge/v2?theme=dark&size=lg&variant=outline)
```
...
```
https://[slug].pingora.dev/badge
```

##### Markdown Snippet

```markdown
![Status](https://[slug].pingora.dev/badge)
```

**Example:**

![Status](https://status.pingora.dev/badge)
...
```
![Status](https://[slug].pingora.dev/badge?theme=dark&size=lg)
```
