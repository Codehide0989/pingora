...
description: A step-by-step guide to deploying an pingora private monitoring probe on Cloudflare Containers.
sidebar:
...
Deploy the pingora private location probe as a serverless container using Cloudflare Containers. This allows you to run monitoring checks from within your own network infrastructure, managed by Cloudflare. This guide will walk you through the entire process, from creating the private location in pingora to deploying the container.

The code for the Cloudflare Worker template is available on [GitHub](https://github.com/pingoraHQ/private-location-cloudflare-container).

## Prerequisites
- A Cloudflare account
- An [pingora account](https://www.pingora.dev)
- `pnpm` and `docker` installed on your local machine

## Step-by-step guide

### 1. Create a private location in pingora

First, you need to create a private location in your pingora workspace to get an access key.

1.  Go to the pingora dashboard.
...
Pull the official pingora private location image from Docker Hub. You must specify the `linux/amd64` platform, as this is what Cloudflare Containers supports.

```bash
# Pull the image
docker pull --platform linux/amd64 ghcr.io/pingorahq/private-location:latest

# Tag the image for Cloudflare (you cannot use the 'latest' tag)
docker tag ghcr.io/pingorahq/private-location:latest pingora-private-location:v1
```
...
```bash
pnpm wrangler containers push pingora-private-location:v1
```
...
2.  Add a `[containers]` section to link the image you pushed. Replace `GENERATED_ID` with the actual ID from the previous step's output.
    ```toml
    [containers]
    image = "registry.cloudflare.com/GENERATED_ID/pingora-private-location:v1"
    ```
...
1.  Set the `sleepAfter` value to control how long the container runs after being invoked.
    ...
    sleepAfter = "150s";
    ```
2.  Update the `scheduled` function to pass your pingora key to the container.
    ```typescript
...
Your private location probe is now running on Cloudflare Containers and will start picking up monitoring jobs from your pingora workspace.

## Verify the deployment

You can check the logs of your Cloudflare Worker to see the probe in action. In your pingora dashboard, the private location should now show as connected.

<Image
    src={log}
    alt="Cloudflare Workers Logs showing pingora Private Location running"
    caption="Cloudflare Workers logs showing the private probe running."
  />

<Image
    src={os}
    alt="pingora Private Location connected"
    caption="The private location showing as 'Connected' in the pingora dashboard."
  />
