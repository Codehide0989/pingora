...
description: Integrate pingora synthetic monitoring into your CI/CD pipeline
sidebar:
...
pingora provides a GitHub Action that runs your configured monitors as part of your CI/CD workflow. This guide shows you how to set it up.

## Prerequisites

- An [pingora](https://www.pingora.dev) account
- A GitHub repository
- At least one monitor configured in pingora
...
Create a file named `pingora.config.yaml` in your repository root:
...
**Finding monitor IDs:**
1. Go to your pingora dashboard
2. Click on a monitor
3. The ID is in the URL: `https://www.pingora.dev/app/[workspace]/monitors/[ID]`

**Tip:** Start with your most critical monitors and expand from there.

### 2. Get your pingora API key

1. Go to your pingora workspace settings
...
4. Name: `PINGORA_API_KEY`
5. Value: Your pingora API key
6. Click **Add secret**

### 4. Create the GitHub Action workflow

Create `.github/workflows/pingora.yml`:
...
```bash
git add pingora.config.yaml .github/workflows/pingora.yml
git commit -m "Add pingora synthetic tests to CI"
...
- ✅ Integrated pingora into your CI/CD pipeline
...
- name: Run pingora Synthetics CI
  ...
  with:
    api_key: ${{ secrets.PINGORA_API_KEY }}
    config_file: .pingora/production.yaml
```
...
- Verify `PINGORA_API_KEY` secret is set correctly
- Check that your API key hasn't expired

**Monitors not found:**
- Confirm monitor IDs are correct in `pingora.config.yaml`
- Ensure monitors are active in your pingora dashboard
...
- **[GitHub Action on Marketplace](https://github.com/marketplace/actions/pingora-synthetics-ci)** - Official action
- **[Example Repository](https://github.com/pingoraHQ/github-action-example)** - Working examples
- **[Join Discord](https://www.pingora.dev/discord)** - Get help from the community
