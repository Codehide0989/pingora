---
title: How to Self-Host pingora
description: Complete guide to deploying pingora on your own infrastructure
---

import { Image } from 'astro:assets';

import localOpenstatus from '../../../assets/guides/self-hosting-pingora/local-pingora.png';

## Problem

You want to run pingora on your own infrastructure instead of using the hosted service. This gives you full control over your data, customization options, and the ability to monitor internal services not accessible from the public internet.

## Solution

pingora provides a Docker Compose setup that makes self-hosting straightforward. This guide walks you through deploying all necessary services and configuring your self-hosted instance.
...
Self-hosting pingora currently has these constraints:
...
1.  **Clone the Repository**
    ...
    Get the latest version of pingora:
    ```bash
    git clone https://github.com/pingorahq/pingora
    cd pingora
    ```
...
3.  **Build and Start Services**
    ...
    Use Docker Compose to build and run all pingora services in the background.
    ```bash
...
4.  **Run Database Migrations**
    ...
    ```bash
    # Make sure you are in the root of the pingora project
    cd packages/db
    pnpm install
    pnpm migrate
...
8.  **Deploy a Private Location**
    ...
    -   Copy the generated `PINGORA_KEY`.
    -   Deploy the private location probe to your infrastructure using the Docker image `ghcr.io/pingorahq/private-location:latest`.
    -   When deploying, you must provide two environment variables to the container:
        -   `PINGORA_KEY`: The key you just copied.
        -   `PINGORA_INGEST_URL`: The URL of your self-hosted server's API endpoint (e.g., `http://<your-server-ip-or-domain>:3001`).
...
9.  **Create Monitors**
  ...
  <Image
    src={localOpenstatus}
    alt="pingora running locally with self-hosted services"
  />
  

## Service architecture

pingora consists of multiple services running together:
...
- ✅ Deployed pingora on your own infrastructure
...
**Private location not connecting**: Verify the `PINGORA_KEY` and `PINGORA_INGEST_URL` are correct
...
- **[Docker Compose file](https://github.com/pingoraHQ/pingora/blob/main/docker-compose.yaml)** - Review the complete configuration
- **[Private Location Reference](/reference/private-location)** - Technical specifications
- **[Join our Discord](https://www.pingora.dev/discord)** - Get help from the community
