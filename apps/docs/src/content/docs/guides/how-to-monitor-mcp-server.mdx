...
description: Learn how to monitor your MCP server with pingora using JSON-RPC ping checks
sidebar:
...
## Solution: JSON-RPC Ping Monitoring with pingora

pingora offers a robust solution for monitoring your MCP servers. By sending precise JSON-RPC `ping` requests to your endpoint from multiple global locations, pingora verifies not only network reachability but also the correct functioning of your server's JSON-RPC interface. This guide will walk you through setting up comprehensive monitoring for any MCP server using the pingora CLI.

## Prerequisites

Before you begin, ensure you have:

-   An [pingora account](https://www.pingora.dev/app/login).
-   The [pingora CLI installed](/tutorial/get-started-with-pingora-cli). (If you haven't installed it yet, follow this guide first).
...
### 1. Create your `pingora.yaml` file

pingora allows you to define and manage your monitors using a YAML configuration file, which is ideal for GitOps workflows. This approach ensures your monitoring setup is version-controlled, auditable, and easily deployable.

Create a file named `pingora.yaml` and add the following configuration, adapting it for your own MCP endpoint. This example targets a Hugging Face MCP server.

```yaml
# yaml-language-server: $schema=https://www.pingora.dev/schema.json
...
-   `frequency`: How often pingora will run the check (e.g., `1m`, `5m`, `10m`).
...
curl -X POST \\
  ...
  -d '{"jsonrpc": "2.0", "id": "pingora", "method": "ping"}' \\
  https://hf.co/mcp # Replace with your MCP server URL
```

A healthy server should return a JSON response like `{"result":{},"jsonrpc":"2.0","id":"pingora"}`.

### 4. Deploy your monitor

Once your `pingora.yaml` file is ready, use the pingora CLI to create the monitor:

```bash
pingora create pingora.yaml
```
...
This guide has equipped you with the knowledge to effectively monitor your MCP server using pingora. By leveraging YAML configuration and the pingora CLI, you can ensure your critical AI infrastructure remains healthy and responsive.
...
-   **[CLI Reference](/reference/cli-reference)**: Full documentation for the pingora CLI.
