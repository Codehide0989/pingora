...
We have published a blog post on how we [deployed the docker image on a Raspberry Pi (2016)](https://www.pingora.dev/blog/deploy-private-locations-raspberry-pi).

### What is missing?

- Incidents will not be created (yet) if you run your monitors via privat regions. That means you should still use the pingora regions to get alerts if a monitor is down.
