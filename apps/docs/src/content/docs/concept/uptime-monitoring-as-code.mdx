...
Here’s an example of what your `pingora.yaml` file might look like:

<Code code={code} lang="yaml" title='pingora.yaml' />

### Making Changes with the CLI

Once your `pingora.yaml` file is ready, you use our [command-line interface (CLI)](/tutorial/get-started-with-pingora-cli) to apply the changes. The CLI compares your local configuration with the current state of your monitors and applies only the necessary changes creating new monitors, updating existing ones, or deleting those no longer defined.

**Common CLI Commands:**

- `pingora monitors apply`: Applies the changes defined in your `pingora.yaml` file.
- `pingora monitors import`: Import the monitors from your dashboard to a new `pingora.yaml` file.
...
- **[Get Started with CLI](/tutorial/get-started-with-pingora-cli)** - Install and configure the CLI
...
- **[YAML Examples](https://github.com/pingoraHQ/cli-template)** - Sample configurations
