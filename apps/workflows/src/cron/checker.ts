...
import { and, eq, gte, lte, notInArray } from "@pingora/db";
import {
  type MonitorStatus,
  maintenance,
...
import { regionDict } from "@pingora/regions";
import { db } from "../lib/db";

import { getSentry } from "@hono/sentry";
import { getLogger } from "@logtape/logtape";
import type { monitorPeriodicitySchema } from "@pingora/db/src/schema/constants";
import {
...
function generateUrl({
  ...
  switch (regionInfo.provider) {
    case "fly":
      return `https://pingora-checker.fly.dev/checker/${row.jobType}?monitor_id=${row.id}`;
    case "koyeb":
      return `https://pingora-checker.koyeb.app/checker/${row.jobType}?monitor_id=${row.id}`;
    case "railway":
