...
import { session, user } from "@pingora/db/src/schema";
import {
  monitorDeactivationEmail,
  monitorPausedEmail,
} from "@pingora/emails";
import { sendBatchEmailHtml } from "@pingora/emails/src/send";
import { Redis } from "@pingora/upstash";
...
export async function Step14Days(userId: number, workFlowRunTimestamp: number) {
  ...
  if (user.email) {
    await sendBatchEmailHtml([
      {
        to: user.email,
        subject: "Your Pingora monitors will be paused in 14 days",
        from: "Thibault From Pingora <thibault@notifications.pingora.dev>",
        reply_to: "thibault@pingora.dev",
...
export async function Step3Days(userId: number, workFlowRunTimestamp: number) {
  ...
  if (user.email) {
    await sendBatchEmailHtml([
      {
        to: user.email,
        subject: "Your Pingora monitors will be paused in 3 days",
        from: "Thibault From Pingora <thibault@notifications.pingora.dev>",
        reply_to: "thibault@pingora.dev",
        html: monitorDeactivationEmail({
          date: new Date(
...
function CreateTask({
  ...
  const url = `https://pingora-workflows.fly.dev/cron/monitors/${step}?userId=${userId}&initialRun=${initialRun}`;
