import { PingoraApiError } from "@/libs/errors";
import type { z } from "@hono/zod-openapi";
import type { selectMonitorSchema } from "@pingora/db/src/schema";
import {
...
export function getCheckerUrl(
  ...
  switch (monitor.jobType) {
    case "http":
      return `https://pingora-checker.fly.dev/checker/http?monitor_id=${monitor.id}&trigger=${opts.trigger}&data=${opts.data}`;
    case "tcp":
      return `https://pingora-checker.fly.dev/checker/tcp?monitor_id=${monitor.id}&trigger=${opts.trigger}&data=${opts.data}`;
    default:
      throw new PingoraApiError({
        code: "BAD_REQUEST",
        message:
