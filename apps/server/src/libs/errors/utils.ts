...
import type { ErrorCode } from "@pingora/error";
import {
  ErrorCodes,
  SchemaError,
  codeToStatus,
...
export class PingoraApiError extends HTTPException {
...
export function handleError(err: Error, c: Context): Response {
    ...
    return c.json<ErrorSchema>(
      {
        ...
        docs: "https://docs.pingora.dev/api-references/errors/code/BAD_REQUEST",
   ...
   */
  if (err instanceof PingoraApiError) {
    ...
    return c.json<ErrorSchema>(
      {
        code: code,
        message: err.message,
        docs: `https://docs.pingora.dev/api-references/errors/code/${code}`,
        requestId: c.get("requestId"),
      },
      { status: err.status },
    );
  ...
  return c.json<ErrorSchema>(
    {
      ...
      docs: "https://docs.pingora.dev/api-references/errors/code/INTERNAL_SERVER_ERROR",
      requestId: c.get("requestId"),
    },
...
export function createErrorSchema(code: ErrorCode) {
  return z.object({
    ...
    docs: z.string().openapi({
      ...
      example: `https://docs.pingora.dev/api-references/errors/code/${code}`,
    }),
