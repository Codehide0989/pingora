...
import { db, eq } from "@pingora/db";
import { selectWorkspaceSchema, workspace } from "@pingora/db/src/schema";
import { apiKey } from "@pingora/db/src/schema/api-keys";
import {
  shouldUpdateLastUsed,
  verifyApiKeyHash,
} from "@pingora/db/src/utils/api-key";
...
export async function authMiddleware(
  ...
  const key = c.req.header("x-pingora-key");
  if (!key)
    throw new PingoraApiError({
      code: "UNAUTHORIZED",
      message: "Missing 'x-pingora-key' header",
    });
