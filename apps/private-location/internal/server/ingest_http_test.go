...
import (
	...
	"github.com/pingorahq/pingora/apps/private-location/internal/server"
	"github.com/pingorahq/pingora/apps/private-location/internal/tinybird"
	private_locationv1 "github.com/pingorahq/pingora/apps/private-location/proto/private_location/v1"
)
...
func TestIngestHTTP_DBError(t *testing.T) {
	...
	req := connect.NewRequest(&private_locationv1.IngestHTTPRequest{})
	req.Header().Set("pingora-token", "token123")
	req.Msg.Id = "monitor1"
	resp, err := h.IngestHTTP(context.Background(), req)
	if err == nil {
...
func TestIngestHTTP_MonitorNotExist(t *testing.T) {
	...
	req := connect.NewRequest(&private_locationv1.IngestHTTPRequest{})
	req.Header().Set("pingora-token", "my-secret-key")
	req.Msg.Id = "monitor1"
	resp, err := h.IngestHTTP(context.Background(), req)
	if err == nil {
		t.Fatalf("expected error for db failure, got nil")
